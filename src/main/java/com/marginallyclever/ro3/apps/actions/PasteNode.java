...
public class PasteNode extends AbstractAction {
    public PasteNode() {
        ...
        ClipboardHelper.getClipboard().addFlavorListener(this::clipboardChanged);
        setEnabled(false);
    }

    private void clipboardChanged(FlavorEvent flavorEvent) {
        // if the clipboard has changed, update the menu item
        if(flavorEvent.getSource() == ClipboardHelper.getClipboard()) {
            if(KeyboardFocusManager.getCurrentKeyboardFocusManager().getActiveWindow() == null) {
                setEnabled(false);
                return;
            }
            try {
                setEnabled(ClipboardHelper.getClipboard().isDataFlavorAvailable(JSONHelper.JSON_FLAVOR));
