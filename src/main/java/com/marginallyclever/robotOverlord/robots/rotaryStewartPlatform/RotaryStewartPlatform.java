...
public class RotaryStewartPlatform extends PoseEntity implements PropertyChangeListener {
	/**
	 * 
	 */
	...
	protected transient Shape modelTop = new Shape("/rotaryStewartPlatform/top.STL");
	protected transient Shape modelBicep = new Shape("/rotaryStewartPlatform/arm.STL");
	protected transient Shape modelBase = new Shape("/rotaryStewartPlatform/base.STL");

	// this should become a list w/ rollback
	...
	protected RotaryStewartPlatformPanel rspPanel;
	...
	public RotaryStewartPlatform() {
		...
		modelTop.setShapeScale(0.1);
		modelBase.setShapeScale(0.1);
		modelBicep.setShapeScale(0.1);
		modelBase.getMaterial().setDiffuseColor(37.0f / 255.0f, 110.0f / 255.0f, 94.0f / 255.0f, 1.0f);
		modelBicep.getMaterial().setDiffuseColor(68.0f / 255.0f, 137.0f / 255.0f, 122.0f / 255.0f, 1.0f);
		modelTop.getMaterial().setDiffuseColor(110.0f / 255.0f, 164.0f / 255.0f, 152.0f / 255.0f, 1.0f);
	}
	...
	public void move(int axis, int direction) {
		/*
		switch (axis) {
		case ActionPhysicalEntityMove.AXIS_X:	xDir = direction;	break;
		case ActionPhysicalEntityMove.AXIS_Y:	yDir = direction;	break;
		case ActionPhysicalEntityMove.AXIS_Z:	zDir = direction;	break;
		case ActionPhysicalEntityMove.AXIS_U:	uDir = direction;	break;
		case ActionPhysicalEntityMove.AXIS_V:	vDir = direction;	break;
		case ActionPhysicalEntityMove.AXIS_W:	wDir = direction;	break;
		}
		*/
	}
	...
	public void rotateFinger() {
		...
		// TODO rotating around itself has no effect.
		result = MathHelper.rotateAroundAxis(forward, of, Math.toRadians(motionFuture.rotationAngleU));
	...
	public void render(GL2 gl2) {
		...
		if (draw_stl) {
			...
			for (i = 0; i < 3; ++i) {
				gl2.glPushMatrix();
				gl2.glTranslated(
						motionNow.arms[i * 2 + 0].shoulder.x, 
				...
				gl2.glPushMatrix();
				gl2.glTranslated(
						motionNow.arms[i * 2 + 1].shoulder.x, 
	...
	public void setModeAbsolute() {
		if (connection.isConnectionOpen())
			connection.sendMessageGuaranteed("G90");
	}

	public void setModeRelative() {
		if (connection.isConnectionOpen())
			connection.sendMessageGuaranteed("G91");
	}
	...
	public void dataAvailable(NetworkSession arg0, String line) {
		if (line.contains(dimensions.hello)) {
			...
			// finalizeMove();
			setModeAbsolute();
				connection.sendMessageGuaranteed("R1");
	...
	private long getNewRobotUID() {
		...
		if (justTestingDontGetUID) {
			...
			if (new_uid != 0) {
				...
				connection.sendMessageGuaranteed("UID " + new_uid);
			}
		}
		return new_uid;
	}
	...
	private void sendChangeToRealMachine() {
		if (connection.isConnectionOpen())
			connection.sendMessage("G0 X" + MathHelper.roundOff3(motionNow.fingerPosition.x) + " Y"
	...
	public void goHome() {
		...
		connection.sendMessageGuaranteed("G28");
