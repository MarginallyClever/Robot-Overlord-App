...
public class RotaryStewartPlatform3MotionState implements Serializable {
	...
	// angle of rotation
	protected RotaryStewartPlatform3Arm arms[];
	...
	public class RotaryStewartPlatform3Arm implements Serializable {
		private static final long serialVersionUID = 1L;
		...
		public void set(RotaryStewartPlatform3Arm other) {
	...
	public RotaryStewartPlatform3MotionState() {
		arms = new RotaryStewartPlatform3Arm[6];
		int i;
		for(i=0;i<6;++i) {
			arms[i] = new RotaryStewartPlatform3Arm();
		}
		...
		this.relative.set(0,0,RotaryStewartPlatform3.STARTING_FINGER_Z
				-RotaryStewartPlatform3.BASE_ADJUST_Z);
	...
	public void set(RotaryStewartPlatform3MotionState other) {
		int i;
		for(i=0;i<6;++i) {
	...
	protected void updateIKEndEffector() {
		...
		// roll, pitch, then yaw
		this.finger_forward = RotaryStewartPlatform3.rotateAroundAxis(this.finger_forward,new Vector3f(1,0,0),Math.toRadians(this.rotationAngleU));
		this.finger_forward = RotaryStewartPlatform3.rotateAroundAxis(this.finger_forward,new Vector3f(0,1,0),Math.toRadians(this.rotationAngleV));
		this.finger_forward = RotaryStewartPlatform3.rotateAroundAxis(this.finger_forward,new Vector3f(0,0,1),Math.toRadians(this.rotationAngleW));

		this.finger_up      = RotaryStewartPlatform3.rotateAroundAxis(this.finger_up,     new Vector3f(1,0,0),Math.toRadians(this.rotationAngleU));
		this.finger_up      = RotaryStewartPlatform3.rotateAroundAxis(this.finger_up,     new Vector3f(0,1,0),Math.toRadians(this.rotationAngleV));
		this.finger_up      = RotaryStewartPlatform3.rotateAroundAxis(this.finger_up,     new Vector3f(0,0,1),Math.toRadians(this.rotationAngleW));

		this.finger_left    = RotaryStewartPlatform3.rotateAroundAxis(this.finger_left,   new Vector3f(1,0,0),Math.toRadians(this.rotationAngleU));
		this.finger_left    = RotaryStewartPlatform3.rotateAroundAxis(this.finger_left,   new Vector3f(0,1,0),Math.toRadians(this.rotationAngleV));
		this.finger_left    = RotaryStewartPlatform3.rotateAroundAxis(this.finger_left,   new Vector3f(0,0,1),Math.toRadians(this.rotationAngleW));
	}
	...
	protected void updateIKWrists() {
		  ...
		  int i;
		  for(i=0;i<3;++i) {
		    RotaryStewartPlatform3Arm arma=this.arms[i*2+0];
		    RotaryStewartPlatform3Arm armb=this.arms[i*2+1];
		    ...
		    arma.wrist.set(n1);
		    arma.wrist.scale(RotaryStewartPlatform3.WRIST_TO_FINGER_Y);
		    arma.wrist.add(this.fingerPosition);
		    temp.set(this.finger_up);
		    temp.scale(-RotaryStewartPlatform3.WRIST_TO_FINGER_Z);
		    arma.wrist.add(temp);
		    armb.wrist.set(arma.wrist);
		    temp.set(o1);
		    temp.scale(RotaryStewartPlatform3.WRIST_TO_FINGER_X);
	...
	protected void updateIKShoulderAngles() throws AssertionError {
		Vector3f ortho = new Vector3f(),w = new Vector3f(),wop = new Vector3f(),temp = new Vector3f(),r = new Vector3f();
		  ...
		  int i;
		  for(i=0;i<6;++i) {
		    RotaryStewartPlatform3Arm arm = this.arms[i];
		    ...
		    b=(float)Math.sqrt(RotaryStewartPlatform3.FOREARM_LENGTH*RotaryStewartPlatform3.FOREARM_LENGTH-a*a);
		    if(Float.isNaN(b)) throw new AssertionError();

		    // use intersection of circles to find elbow point.
		    //a = (r0r0 - r1r1 + d*d ) / (2*d) 
		    r1=b;  // circle 1 centers on wrist
		    r0=RotaryStewartPlatform3.BICEP_LENGTH;  // circle 0 centers on shoulder
		    d=wop.length();
...
//		    armb.shoulder = n1*BASE_TO_SHOULDER_X + motion_future.base_up*BASE_TO_SHOULDER_Z + o1*BASE_TO_SHOULDER_Y;
		    ...
		    arma.shoulder.scale(RotaryStewartPlatform3.BASE_TO_SHOULDER_X);
		    temp.set(this.baseUp);
		    temp.scale(RotaryStewartPlatform3.BASE_TO_SHOULDER_Z);
		    arma.shoulder.add(temp);
		    armb.shoulder.set(arma.shoulder);
		    temp.set(o1);
		    temp.scale(RotaryStewartPlatform3.BASE_TO_SHOULDER_Y);
...
//		    armb.elbow = n1*BASE_TO_SHOULDER_X + motion_future.base_up*BASE_TO_SHOULDER_Z + o1*(BASE_TO_SHOULDER_Y+BICEP_LENGTH);
		    arma.elbow.set(n1);
		    arma.elbow.scale(RotaryStewartPlatform3.BASE_TO_SHOULDER_X);
		    temp.set(this.baseUp);
		    temp.scale(RotaryStewartPlatform3.BASE_TO_SHOULDER_Z);
		    arma.elbow.add(temp);
		    armb.elbow.set(arma.elbow);
		    temp.set(o1);
		    temp.scale(RotaryStewartPlatform3.BASE_TO_SHOULDER_Y+RotaryStewartPlatform3.BICEP_LENGTH);
		    arma.elbow.sub(temp);
		    armb.elbow.add(temp);
		    //arma.shoulder.add(this.base);
		    //armb.shoulder.add(this.base);		    
...
*/
	...
	public boolean checkAngleLimits() {
		...
		if (state.angle_5 >  180) return false;
		*/
		if(Math.abs(rotationAngleU)>RotaryStewartPlatform3.LIMIT_U) return false;
		if(Math.abs(rotationAngleV)>RotaryStewartPlatform3.LIMIT_V) return false;
		if(Math.abs(rotationAngleW)>RotaryStewartPlatform3.LIMIT_W) return false;
