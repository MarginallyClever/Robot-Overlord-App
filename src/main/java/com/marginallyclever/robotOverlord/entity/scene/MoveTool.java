	...
	public void renderRotation(GL2 gl2) {
		...
		if(isActivelyMoving && activeMoveIsRotation) {
			...
			for(double i=0;i<absRange;i+=0.01) {
				...
				switch(nearestPlane) {
				case X: gl2.glVertex3d(0,Math.cos(n+Math.PI/2)*radius0,Math.sin(n+Math.PI/2)*radius0);  break;
				case Y: gl2.glVertex3d(Math.cos(-n)*radius0,0,Math.sin(-n)*radius0);  break;
				case Z: gl2.glVertex3d(Math.cos( n)*radius0,Math.sin( n)*radius0,0);  break;
				}
			}
			gl2.glEnd();
		}
