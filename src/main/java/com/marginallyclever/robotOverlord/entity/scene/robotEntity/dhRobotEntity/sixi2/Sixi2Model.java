...
public abstract class Sixi2Model extends DHRobotEntity {	
	...
	public Sixi2Model() {
		...
		// If I have the poseWorld for each DHLink then I can use that to adjust the model values.
	...
	// https://www.youtube.com/watch?v=f79suBeEObs
	public void NEW_FORM() {
		...
		if(attach) links.get(0).setModelFilename("/Sixi2/shoulder.stl");
		...
		if(attach) links.get(1).setModelFilename("/Sixi2/bicep.stl");
		links.get(1).setD(0);
		...
		if(attach) links.get(2).setModelFilename("/Sixi2/forearm.stl");
		links.get(2).setD(0);
		...
		if(attach) links.get(3).setModelFilename("/Sixi2/tuningFork.stl");
		links.get(3).setD(0);
		...
		if(attach) links.get(4).setModelFilename("/Sixi2/picassoBox.stl");
		...
		if(attach) links.get(5).setModelFilename("/Sixi2/hand.stl");
