...
public class Arm3RobotMotionState {
	// angle of rotation
	...
	public Vector3f fingerPosition = new Vector3f(Arm3Robot.HOME_X,Arm3Robot.HOME_Y,Arm3Robot.HOME_Z);
	public Vector3f fingerForward = new Vector3f(Arm3Robot.HOME_FORWARD_X,Arm3Robot.HOME_FORWARD_Y,Arm3Robot.HOME_FORWARD_Z);
	public Vector3f fingerRight = new Vector3f(Arm3Robot.HOME_RIGHT_X,Arm3Robot.HOME_RIGHT_Y,Arm3Robot.HOME_RIGHT_Z);
	public float iku=0;
	public float ikv=0;
	public float ikw=0;
	...
	public void set(Arm3RobotMotionState other) {
		angleA = other.angleA;
		angleB = other.angleB;
		angleC = other.angleC;
		iku=other.iku;
	...
	public boolean movePermitted() {
		...
		// check near limit
		if(temp.length() < Arm3Robot.BASE_TO_SHOULDER_MINIMUM_LIMIT) return false;
	...
	protected boolean IK() {
		...
		this.wrist.scale(-Arm3Robot.WRIST_TO_FINGER);
	    ...
	    this.shoulder.scale(Arm3Robot.BASE_TO_SHOULDER_X);
	    this.shoulder.z = Arm3Robot.BASE_TO_SHOULDER_Z;
	    
	    // use intersection of circles to find elbow
	    Vector3f es = new Vector3f(this.wrist);
	    es.sub(this.shoulder);
	    float d = es.length();
	    float r1=Arm3Robot.ELBOW_TO_WRIST;  // circle 1 centers on wrist
	    float r0=Arm3Robot.SHOULDER_TO_ELBOW;  // circle 0 centers on shoulder
	    if( d > Arm3Robot.ELBOW_TO_WRIST + Arm3Robot.SHOULDER_TO_ELBOW ) {
	...
	protected void FK() {
		Vector3f arm_plane = new Vector3f((float)Math.cos(Math.toRadians(this.angleA)),
					  					  ...
					  					  0);
		this.shoulder.set(arm_plane.x*Arm3Robot.BASE_TO_SHOULDER_X,
						   arm_plane.y*Arm3Robot.BASE_TO_SHOULDER_X,
						   Arm3Robot.BASE_TO_SHOULDER_Z);
		
		this.elbow.set(arm_plane.x*(float)Math.cos(-Math.toRadians(this.angleB))*Arm3Robot.SHOULDER_TO_ELBOW,
						arm_plane.y*(float)Math.cos(-Math.toRadians(this.angleB))*Arm3Robot.SHOULDER_TO_ELBOW,
									(float)Math.sin(-Math.toRadians(this.angleB))*Arm3Robot.SHOULDER_TO_ELBOW);
		this.elbow.add(this.shoulder);

		this.wrist.set(arm_plane.x*(float)Math.cos(Math.toRadians(this.angleC))*-Arm3Robot.ELBOW_TO_WRIST,
				 		arm_plane.y*(float)Math.cos(Math.toRadians(this.angleC))*-Arm3Robot.ELBOW_TO_WRIST,
				 					(float)Math.sin(Math.toRadians(this.angleC))*-Arm3Robot.ELBOW_TO_WRIST);
		...
		this.fingerPosition.scale(Arm3Robot.WRIST_TO_FINGER);
