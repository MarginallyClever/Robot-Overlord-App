...
public class DeltaRobot2MotionState {
	// angle of rotation
	DeltaRobot2Arm arms[];
	...
	public class DeltaRobot2Arm {
		...
		public void set(DeltaRobot2Arm other) {
	...
	public DeltaRobot2MotionState() {
		arms = new DeltaRobot2Arm[NUM_ARMS];
		int i;
		for(i=0;i<NUM_ARMS;++i) {
			arms[i] = new DeltaRobot2Arm();
		}
	}

	public void set(DeltaRobot2MotionState other) {
	...
	protected void updateIKWrists() {
		...
		int i;
		for(i=0;i<NUM_ARMS;++i) {
			DeltaRobot2Arm arma=this.arms[i];
			...
			arma.wrist.set(n1);
			arma.wrist.scale(DeltaRobot2.WRIST_TO_FINGER_X);
			arma.wrist.add(this.finger_tip);
			temp.set(this.finger_up);
			temp.scale(DeltaRobot2.WRIST_TO_FINGER_Z);
			arma.wrist.add(temp);
			temp.set(o1);
			temp.scale(DeltaRobot2.WRIST_TO_FINGER_Y);
			arma.wrist.sub(temp);
		}
	}
	...
	protected void updateIKShoulderAngles() throws AssertionError {
		...
		int i;
		for(i=0;i<NUM_ARMS;++i) {
			DeltaRobot2Arm arm = this.arms[i];
			...
			b=(float)Math.sqrt(DeltaRobot2.FOREARM_LENGTH*DeltaRobot2.FOREARM_LENGTH-a*a);
			if(Float.isNaN(b)) throw new AssertionError();

			// use intersection of circles to find elbow point.
			//a = (r0r0 - r1r1 + d*d ) / (2*d) 
			r1=b;  // circle 1 centers on wrist
			r0=DeltaRobot2.BICEP_LENGTH;  // circle 0 centers on shoulder
			d=wop.length();
	...
	protected void rebuildShoulders() {
		...
		for(i=0;i<3;++i) {
			...
			arma.shoulder.scale(DeltaRobot2.BASE_TO_SHOULDER_X);
			temp.set(this.base_up);
			temp.scale(DeltaRobot2.BASE_TO_SHOULDER_Z);
			arma.shoulder.add(temp);
			temp.set(o1);
			temp.scale(DeltaRobot2.BASE_TO_SHOULDER_Y);
			...
			arma.elbow.set(n1);
			arma.elbow.scale(DeltaRobot2.BASE_TO_SHOULDER_X);
			temp.set(this.base_up);
			temp.scale(DeltaRobot2.BASE_TO_SHOULDER_Z);
			arma.elbow.add(temp);
			temp.set(o1);
			temp.scale(DeltaRobot2.BASE_TO_SHOULDER_Y+DeltaRobot2.BICEP_LENGTH);
			arma.elbow.sub(temp);
			//arma.shoulder.add(this.base);		    
