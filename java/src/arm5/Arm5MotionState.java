...
class Arm5MotionState {
	...
	public boolean movePermitted() {
		// don't hit floor
		if(fingerTip.z<0.25f) {
			return false;
		}
		// don't hit ceiling
		if(fingerTip.z>50.0f) {
			return false;
		}

		// check far limit
		Vector3f temp = new Vector3f(fingerTip);
		temp.sub(shoulder);
	...
	protected boolean inverseKinematics() {
	    ...
	    Vector3f finger = new Vector3f(fingerTip);
	...
	protected void forwardKinematics() {
		...
		fingerTip.set(arm_plane);
		fingerTip = Arm5Robot.rotateAroundAxis(fingerTip, axis, Math.toRadians(-angleB) ); 
		fingerTip = Arm5Robot.rotateAroundAxis(fingerTip, fn, Math.toRadians(-angleA) );
		fingerTip.scale(Arm5Robot.WRIST_TO_FINGER);
		fingerTip.add(wrist);

		fingerForward.set(fingerTip);
